...
void validation(string type="ByChi2", bool isRunI = false ){
...
//   gROOT->SetStyle("Plain");
  ...
  if (type=="ByChi2"){
    if (isRunI == true){
      filename.push_back("../valid_NS_Zp8TeV_chi2_runI.root");
      filename.push_back("../valid_TS_Zp8TeV_chi2_runI.root");
      filename.push_back("../valid_SS_Zp8TeV_chi2_runI.root");
    } else {
      filename.push_back("DQM_V0001_R000000001__ZpTauTau__7_1_6__nosplit.root");
      filename.push_back("DQM_V0001_R000000001__ZpTauTau__7_1_6__split.root");
      filename.push_back("DQM_V0001_R000000001__ZpTauTau__7_1_6__simsplit.root");
    }
  }else if (type=="ByHits"){
...
//   vHistoName.push_back("fakerate_vertcount_fwdpos");
  ...
  for (unsigned int ifile = 0 ; ifile < filename.size(); ifile++){
    ...
    for (unsigned int iname=0; iname<vHistoName.size(); iname++){
      TDirectory * dir;
      if (type == "ByChi2" ){
	dir= (TDirectory * ) file->Get("DQMData/Run 1/Tracking/Run summary/Track/SPLIT_general_AssociatorByChi2");
  ...
  TLegend * leg ;
  if (type == "ByChi2") {
    leg = new TLegend (0.5,0.55, 0.95,0.4, "","brNDC");
  }else {
  ...
  for (unsigned int iname=0; iname<vHistoName.size(); iname++){
	...
	histoMap[ifile][iname]->Draw("E"); 
	if (((string)histoMap[ifile][iname]->GetName()).find("effic") < 10) { 
	  histoMap[ifile][iname]->GetYaxis()->SetRangeUser(0.5,1.05);
	}else {
	  histoMap[ifile][iname]->GetYaxis()->SetRangeUser(0.,0.5);
	}
	...
	histoMap[ifile][iname]->SetLineWidth(3);
	histoMap[ifile][iname]->SetMarkerStyle(20);
	histoMap[ifile][iname]->SetMarkerColor(icolor);
	histoMap[ifile][iname]->SetMarkerSize(0.8);
...
// 	vH[iname] ->GetName(); 
	...
	histoMap[ifile][iname]->Draw("SAMESE");
	histoMap[ifile][iname]->SetLineColor(icolor);
	histoMap[ifile][iname]->SetLineWidth(3);
	histoMap[ifile][iname]->SetMarkerStyle(20);
	histoMap[ifile][iname]->SetMarkerSize(0.8);
    ...
    TPaveText* paves = new TPaveText(0.,1.1,0.009,1.05);
    paves->AddText("CMS Simulation"); 

    TPaveText* paves2 = new TPaveText(0.019,1.1,0.025,1.05);
    paves2->AddText("#sqrt{s} = 8 TeV"); 
    paves->SetFillColor(0);
    paves2->SetFillColor(0);
    paves->SetBorderSize(0);
    paves2->SetBorderSize(0);
    paves->SetTextSize(0.04); 
    paves2->SetTextSize(0.04); 
    ...
    stringstream cname; 
    cname.str(""); 
    std::size_t found =  filename[0].find("runI");
    if (found!=std::string::npos) cname <<"runI_";
