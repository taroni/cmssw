...
MTVHistoProducerAlgoForTracker::MTVHistoProducerAlgoForTracker(const edm::ParameterSet& pset, edm::ConsumesCollector & iC): MTVHistoProducerAlgo(pset, iC){
  ...
  TpSelectorForEfficiencyVsdR     = new TrackingParticleSelector(ParameterAdapter<TrackingParticleSelector>::make(TpSelectorForEfficiencyVsdRPSet, iC));
...
void MTVHistoProducerAlgoForTracker::bookRecoHistos(DQMStore::IBooker& ibook){
  ...
  ///dRhisto
  h_efficdR.push_back( ibook.book1D("efficdR","efficiency vs dR",nintdR,mindR,maxdR) );
  h_fakeratedR.push_back( ibook.book1D("fakeratedR","fakerate vs dR",nintdR,mindR,maxdR) );
  //pt vs dR
  ptres_vs_dR.push_back(ibook.book2D("ptres_vs_dR","ptres_vs_dR",nintdR,mindR,maxdR, ptRes_nbin, ptRes_rangeMin, ptRes_rangeMax));
  h_ptmeanhdR.push_back( ibook.book1D("ptres_vs_dR_Mean","mean of p_{t} resolution vs dR",nintdR,mindR,maxdR));
  h_ptrmshdR.push_back( ibook.book1D("ptres_vs_dR_Sigma","#sigma(#deltap_{t}/p_{t}) vs dR",nintdR,mindR,maxdR) );
  //phires vs dR
  phires_vs_dR.push_back(ibook.book2D("phires_vs_dR","phires_vs_dR",nintdR,mindR,maxdR, phiRes_nbin, phiRes_rangeMin, phiRes_rangeMax));
  h_phimeanhdR.push_back(ibook.book1D("phires_vs_dR_Mean","mean of #phi res vs dR",nintdR,mindR,maxdR));
  h_phirmshdR.push_back( ibook.book1D("phires_vs_dR_Sigma","#sigma(#delta#phi) vs dR",nintdR,mindR,maxdR) );
  //cotTheta vs dR
  cotThetares_vs_dR.push_back(ibook.book2D("cotThetares_vs_dR","cotThetares_vs_dR",nintdR,mindR,maxdR, cotThetaRes_nbin, cotThetaRes_rangeMin, cotThetaRes_rangeMax));      
  h_cotThetameanhdR.push_back( ibook.book1D("cotThetares_vs_dR_Mean","#sigma(cot(#theta)) vs dR Mean",nintdR,mindR,maxdR) );
  h_cotThetarmshdR.push_back( ibook.book1D("cotThetares_vs_dR_Sigma","#sigma(cot(#theta)) vs dR Sigma",nintdR,mindR,maxdR) );
  //dxy vs dR
  dxyres_vs_dR.push_back( ibook.book2D("dxyres_vs_dR","dxyres_vs_dR",nintdR,mindR,maxdR,dxyRes_nbin, dxyRes_rangeMin, dxyRes_rangeMax));
  h_dxymeanhdR.push_back( ibook.book1D("dxyres_vs_dR_Mean","mean of dxyres vs dR",nintdR,mindR,maxdR));
  h_dxyrmshdR.push_back( ibook.book1D("dxyres_vs_dR_Sigma","#sigmadxy vs dR",nintdR,mindR,maxdR));
  //dz vs dR
  dzres_vs_dR.push_back(ibook.book2D("dzres_vs_dR","dzres_vs_dR",nintdR,mindR,maxdR,dzRes_nbin, dzRes_rangeMin, dzRes_rangeMax));
  h_dzmeanhdR.push_back( ibook.book1D("dzres_vs_dR_Mean","mean of dzres vs dR",nintdR,mindR,maxdR) );
  h_dzrmshdR.push_back( ibook.book1D("dzres_vs_dR_Sigma","#sigma(#deltadz vs dR)",nintdR,mindR,maxdR) );
...
void MTVHistoProducerAlgoForTracker::fill_ResoAndPull_recoTrack_histos(int count,
								       ...
								       int chargeTP,
								       const reco::Track& track,
								       const math::XYZPoint& bsPosition,edm::Handle<edm::View<reco::Track> >  trackCollection){
